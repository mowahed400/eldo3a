                        ...
                        <div class="col-12">
                            ...
                            <div class="breadcrumb-wrapper">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item">
                                        <a
                                            href="{{ route('admin.contents.index') }}"> {{__('messages.static.list',['name'=> trans_choice('labels.models.content',2)])}}</a>
                                    </li>
                                            ...
                                            <div class="row">
                                                <div class="col-12 mb-2">
                                                    <div class="form-group">
                                                        <label
                                                            for="section">{{ trans_choice('labels.models.section',1) }}
                                                            <span class="text-danger">*</span>
                                                        </label>

                                                        <select name="section_id" id="section" class="form-control select2"
                                                                required>
                                                            <option value="{{old('section_id',$content->section_id)}}">
                                                                --{{$content->section->translate('name','ar')}}--
                                                            </option>

                                                            @foreach($sections as $section)
                                                                <option
                                                                    value="{{$section->id}}">{{$section->translate('name','ar')}}</option>
                                                            @endforeach
                                                        </select>
...
@push('js')
    ...
    <script>
        let categoryId = {{$content->category_id}};
        ...
        let selectCategories = (id = null) => {
            let $id = id ?? $('#section').find(":selected").val();
            if ($id) {
                ...
                $.ajax({
                    ...
                    success: function ({categories}) {
                        ...
                        for (let i = 0; i < categories.length; i++) {
                            ...
                            {
                                $('#category_id').append(
                                    `<option value="${categories[i].id}" selected>${categories[i].name.ar}</option>`
                                );
                                continue;
                            }
                            $('#category_id').append(
        ...
        selectCategories({{$content->section_id}});
